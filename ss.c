...
int read_paths_from_file(const char *filename, char *paths[])
{
    // Open input file
    ...
    // Read base directory (first line)
    if (fgets(buffer, sizeof(buffer), file) == NULL)
    {
        fprintf(stderr, "Error reading base directory\n");
        fclose(file);
        return -1;
    }

    // Remove newline from base directory
    buffer[strcspn(buffer, "\n")] = 0;

    // Ensure base directory starts with ./
    if (strncmp(buffer, "./", 2) != 0)
    {
        // Allocate a temporary buffer to construct the corrected path
    ...
    paths[path_count] = strdup(buffer);
    if (paths[path_count] == NULL)
    {
        perror("Error allocating memory for base directory");
        fclose(file);
        return -1;
    }
    path_count++;

    // Create the /backup folder in the base directory
    char backup_path[MAX_PATH_LENGTH];
    snprintf(backup_path, sizeof(backup_path), "%s/backup", buffer);
    ...
    // Add the /backup path to the list
    paths[path_count] = strdup(backup_path);
    if (paths[path_count] == NULL)
    {
        perror("Error allocating memory for /backup path");
        fclose(file);
        return -1;
    }
    path_count++;

    // Read and process remaining paths
    while (fgets(buffer, sizeof(buffer), file) && path_count < MAX_PATHS)
    {
        // Remove newline
        buffer[strcspn(buffer, "\n")] = 0;

        // Construct full path by concatenating base directory
        char full_path[MAX_PATH_LENGTH];
        snprintf(full_path, sizeof(full_path), "%s%s", paths[0], buffer);

        // Allocate memory and copy full path
...
int main()
{
    ...
    // char *paths[] = {"/home/sigullapalliakash/Documents/osn_eval/course-project-team_17-main/root_directory/dir_gywnw/dir_fzxpq/dir_wewny/dir_juqnm/big_dir_test/big_file_70MB.txt"};
    ...
    int path_count = read_paths_from_file(filename, paths);
    int num_paths = path_count;
    // convert_to_absolute_paths(paths, num_paths);
